...
import { babySittingPackage } from "../../../data/services/babysitting";
import CTAButton from "../../../components/ui/CTAButton";
import Icon from "../../../components/ui/Icon";

const BabySittingPackageCards = () => {
  // Set default selected package to the first item in babySittingPackage
  const [selectedTier, setSelectedTier] = useState(
    babySittingPackage[0]?.tier || "",
  );

  // Find active package object based on selection
  const activePkg =
    babySittingPackage.find((pkg) => pkg.tier === selectedTier) ||
    babySittingPackage[0];
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Professional, vetted child care tailored to your family's
                schedule
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
            {/* LEFT COLUMN: Stretched Top-to-Bottom Buttons */}
            <div className="lg:col-span-5 flex flex-col justify-between gap-3 h-full">
              {babySittingPackage.map((pkg) => {
            ...
            {/* RIGHT COLUMN: Active Details & Features/Description */}
            <div className="lg:col-span-7 bg-[#0E1420] border border-slate-800 rounded-2xl p-6 flex flex-col justify-between">
              <div>
                <div className="flex items-start justify-between gap-3 mb-3 pb-3 border-b border-slate-800/80">
                  ...
                  <div className="text-right flex-shrink-0">
                    {activePkg?.from && (
                      <span className="block text-[10px] font-bold uppercase text-[#60A5FA] tracking-wider">
                        From
                      </span>
                    )}
              ...
              <div className="pt-3 border-t border-slate-800/80 flex justify-center">
                <CTAButton label="Book a Baby Sitter" serviceId="babysitting" />
              </div>
            </div>
          </div>
        </div>
...
export default BabySittingPackageCards;
