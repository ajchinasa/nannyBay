...
const DriverPackageCards = () => {
  // Set default selected package to the first item in driverServicePackages
  const [selectedTier, setSelectedTier] = useState(
    driverServicePackages[0]?.tier || "",
  );

  // Find active package object based on selection
  const activePkg =
    driverServicePackages.find((pkg) => pkg.tier === selectedTier) ||
    driverServicePackages[0];
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300 mt-1 font-normal leading-relaxed">
                Experienced, licensed chauffeurs ensuring safe, punctual travel
                for your daily routines.
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
            {/* LEFT COLUMN: Stretched Top-to-Bottom */}
            ...
            <div className="lg:col-span-7 bg-[#0E1420] border border-slate-800 rounded-2xl p-6 flex flex-col justify-between">
              ...
              <div className="pt-3 border-t border-slate-800/80 flex justify-center">
                <CTAButton label="Book a Driver" serviceId="driver-services" />
              </div>
            </div>
          </div>
        </div>
